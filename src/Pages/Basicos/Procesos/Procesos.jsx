...
const Procesos = () => {
  ...
  const { dataProcesos, buscar_proceso, setPermisosProcesos, permisosProcesos } = useProcesos()
    ...
    <>
      {
        permisosProcesos.length === 0 ?
          (<Loader />) :
          (permisosProcesos.filter(permiso => permiso.permiso.toLowerCase() === Permisos_DB.CONSULTAR).length > 0
            ?
            (main())
            :
            (<Forbidden />))
