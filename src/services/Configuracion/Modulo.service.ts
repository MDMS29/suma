...
export default class ModuloService {
    ...
    public async Obtener_Modulos(estado: number): Promise<any> {
        ...
        try {
            ...
            if (!modulos) {
                return {
                    error: true,
                    message: `No hay módulos ${estado == EstadosTablas.ESTADO_ACTIVO ? 'activos' : 'inactivos'}`
                } //!ERROR
            }
            return modulos
        } catch (error) {
    ...
    public async Insertar_Modulo(modulo_request: ModulosUsuario, usuario_creador: string, roles: any): Promise<any> {
        ...
        try {
            ...
            const log = await this._Querys.Insertar_Log_Auditoria(usuario_creador, modulo_request.ip, modulo_request?.ubicacion)
            if (log !== 1) {
                console.log(`ERROR AL INSERTAR LOGS DE AUDITORIA: USUARIO: \n ${usuario_creador}, IP: \n ${modulo_request.ip}, UBICACIÓN: \n ${modulo_request?.ubicacion}`)
            }
            ...
            for (let rol of roles) {
                // AGREGAR INFORMACION DEL USUARIO PARA INSERTAR LOG DE AUDITORIA
                const log_2 = await this._Querys.Insertar_Log_Auditoria(usuario_creador, modulo_request.ip, modulo_request?.ubicacion)
                if (log_2 !== 1) {
                    console.log(`ERROR AL INSERTAR LOGS DE AUDITORIA: USUARIO: \n ${usuario_creador}, IP: \n ${modulo_request.ip}, UBICACIÓN: \n ${modulo_request?.ubicacion}`)
                }
        ...
        try {
            ...
            if (moduloB.icono !== Request_Modulo.icono) {
                if (!Request_Modulo.icono.startsWith('pi-')) {
                    return {
                        error: true,
                        message: 'Este icono es invalido, ingrese a: https://primereact.org/icons/#list'
                    }
                }
            ...
            // AGREGAR INFORMACION DEL USUARIO PARA INSERTAR LOG DE AUDITORIA
            const log = await this._Querys.Insertar_Log_Auditoria(usuario_modi, Request_Modulo.ip, Request_Modulo?.ubicacion)
            if (log !== 1) {
                console.log(`ERROR AL INSERTAR LOGS DE AUDITORIA: USUARIO: \n ${usuario_modi}, IP: \n ${Request_Modulo.ip}, UBICACIÓN: \n ${Request_Modulo?.ubicacion}`)
            }
