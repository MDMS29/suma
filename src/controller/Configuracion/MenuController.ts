    ...
    public async Cambiar_Estado_Menu(req: Request, res: Response) {
        ...
        try {
            ...
            return res.status(200).json({
                error: false,
                message: +estado === EstadosTablas.ESTADO_ACTIVO ? 'Se ha activado el menu' : 'Se ha desactivado el menu'
            }) //*SUCCESSFUL
            ...
            return res.status(500).json({
                error: true,
                message: +estado === EstadosTablas.ESTADO_ACTIVO ? 'Error al activar el menu' : 'Error al desactivar el menu'
            }) //!ERROR
        }
    }
