...
const ModalAsignarMenu = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    const moduloEncontrado = dataModulos.find(
      (modulo) => modulo.id_modulo === ModuloState
    );
    if (moduloEncontrado) {
      setModuloNombre(moduloEncontrado.nombre_modulo);
    }
    let enlace = `${moduloEncontrado?.nombre_modulo}/${MenusAgg.nombre_menu}`
      .split(" ")
      .join("-");
