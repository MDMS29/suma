...
const ModalRevisarReq = ({ visible, onClose }) => {
  ...
  const chk_producto = (rowData) => {
    ...
    if (
      productosSeleccionados.find(
        (producto) => producto.id_detalle == producto_id
      )
    ) {
      const [productos] = productosSeleccionados.filter(
        (producto) => producto.id_detalle == producto_id
      );
      ...
      const productosActualizados = productosSeleccionados.map(
        (productoState) =>
          productoState.id_detalle == productos.id_detalle
            ? productos
            : productoState
      );
    ...
    <label
      className={`p-checkbox w-10 h-5 cursor-pointer relative rounded-full ${
        productosSeleccionados.filter(
          (producto) => producto.id_detalle == rowData.id_detalle
        )[0]?.id_estado === 4
          ? "bg-primaryYellow"
          : "bg-gray-300"
      }`}
  ...
  const header = (
    <>
      <div className="flex gap-3 m-3 items-center">
        ...
        <label
          className={`p-checkbox w-10 h-5 cursor-pointer relative rounded-full ${
            !productosSeleccionados.some((producto) => producto.id_estado == 5)
              ? "bg-primaryYellow"
              : "bg-gray-300"
          }`}
        >
  ...
  const footerContent = (
    <div className="mt-3">
      <Button tipo={"PRINCIPAL"} funcion={guardar_lista}>
        {" "}
        Confirmar
      </Button>
    </div>
  ...
  return (
    <Dialog
      header={
        <h1>
          Revision Requisicion{" "}
          <label className="font-sans text-2xl">{RequiAgg.consecutivo}</label>
        </h1>
      }
      visible={visible}
      onHide={onClose}
      ...
      <div className="flex flex-col pt-3 flex-wrap sm:w-full">
        <div className="grid grid-cols-3 gap-4">
          <div className="flex flex-col">
            ...
            <p name="id_proceso" className="w-full text-base">
              {RequiAgg.proceso}
            </p>
          </div>
          <div className="flex flex-col">
            ...
            <p name="id_centro" className="w-full text-base">
              {RequiAgg.centro}
            </p>
          </div>
          <div className="flex flex-col">
            <label className="text-gray-600 pb-2 font-semibold">Fecha</label>
            <p name="fecha_requisicion" className="w-full text-base">
              {RequiAgg.fecha_requisicion}
            </p>
          </div>
          <div className="flex flex-col">
            <label className="text-gray-600 pb-2 font-semibold">
            ...
            <p name="id_tipo_producto" className="w-full text-base">
              {RequiAgg.tipo_productos}
            </p>
          </div>
