...
import { _BuscarRolID, _BuscarRolNombre, _InsertarRol, _ObtenerRoles, _ObtenerUltimoID } from "../dao/DaoRol";
...
export default class QueryRol {
    ...
    public async BuscarRolID(nombre_rol: string) {
        try {
            let result = await client.query(_BuscarRolID, [nombre_rol]);
            return result.rows
        } catch (error) {
            console.log(error)
            return
